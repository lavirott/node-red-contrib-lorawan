...
// var RN2483 = require('../node-red-contrib-lorawan-rn2483/lib/commands.js');
// response="RN2483 1.0.3 Mar 22 2017 06:00:42"
// searchResponse("sys get ver", response)
  // start = 0;
  // do {
    // ind = response.indexOf(' ', start);
    // if (ind =! -1)
      // answer = response.substr(0, ind);
    // else
      // answer = response
    // console.log('|' + answer + '|');
    // start = ind + 1;
  // } while ( (RN2483.commands[cmd][answer] == 'undefined') || (ind != -1) )
  // return answer;
// }
...
LoRaWAN.prototype._initSerial = function () {
  ...
  this.serialConnection.parser = this.serialConnection.pipe(new Delimiter({ delimiter: '\r\n'}));
    ...
    try {
      ...
      me.serialConnection.write(data + "\r\n", function (err, res) {
        if (err) {
          ...
          me.callbackStack.splice(me.callbackStack.indexOf({data, callback}), 1);
          callback(err);
        };
      });
      me.callbackStack.unshift({data, callback});
